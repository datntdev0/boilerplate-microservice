...
1. Create a new migration using the Entity Framework Core CLI on the `Migrator` project directory:
   ```bash
   ...
   ```
2. Apply the migration to the database by running the `Migrator` project, which includes schema migrations and data seeding.

## Squash migrations into a single migration 

> WARN: This will delete all existing migration history and data in the database, so use with caution
